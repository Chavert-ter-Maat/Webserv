...
auto	print_key_value = [](const auto &key,
			const auto &value) { std::cout << "Key:[" << key << "] Value:[" << value << "]\n"; };
...
Request::Request(const Request &src) : _rawRequest(src._rawRequest),
	_requestMethod(src._requestMethod), _uri(src._uri),
	_htmlVersion(src._htmlVersion), _requestArgs(src._requestArgs),
	_headers(src._headers), _keepAlive(src._keepAlive), _body(src._body),
...
void Request::extractCgiEnv() // whoops wrong place, also security issue :p
{
	if (get_headers().empty())
		return ;
	// Should check for accepted env variables,
	// but dont think its specified to be that way.
	...
	while (std::getline(requestStream, line) && !line.empty() && line != "\r")
	{
		...
		if (pos != std::string::npos)
		{
			...
			for (auto &c : headerKey)
				c = tolower(c);
			headerValue = line.substr(headerPos, line.find_last_not_of("/r")
					- headerPos);
...
const std::string &Request::get_rawRequest() const
{
	return (_rawRequest);
}

const std::string &Request::get_requestMethod() const
{
...
const bool &Request::get_keepAlive() const
{
	return (_keepAlive);
}

const std::string &Request::get_htmlVersion() const
{
	return (_htmlVersion);
}
...
void Request::printRequest() const
{
	...
	(keepAlive ? std::cout << "Keep-Alive: true" << std::endl : std::cout << "Keep-Alive: false" << std::endl);

	std::cout << "<Body>" << std::endl;
	std::string body = get_body();
	(body.empty()) ? std::cout << "Empty Body" << std::endl : std::cout << body << std::endl;

	std::cout << "<Request Validity>" << std::endl;
	(checkRequestValidity() == true) ? (std::cout << "Valid!" << std::endl) : (std::cout << "Invalid!" << std::endl);
#endif // DEBUG
